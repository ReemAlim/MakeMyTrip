...
public class BasePage {
    ...
    protected void getLiDropdownOption(By ulLocator, By liLocator, By pLocator, String text) {
        ...
        for (WebElement op : liOptions) {
            try {
                if (webDriverWait.until(ExpectedConditions.visibilityOf(op.findElement(pLocator))).getText().equalsIgnoreCase(text)) {
                    try {
                        webDriverWait.until(ExpectedConditions.visibilityOf(op.findElement(pLocator))).click();
                        break;
